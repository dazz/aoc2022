...
function walkSize(System $system, $line): System
{
    $tree = $system->tree;

    return new System($tree);
}
function walk(System $system, $line): System
{
    ...
    foreach ($tree as $path => $dir) {
        $tree[$path]['spaceNeeded'] = $tree['/']['size'] - 40000000;
    }
...
$result = pipe($inputFile,
    ...
    afilter(fn($dir) => $dir['size'] > $dir['spaceNeeded']),
